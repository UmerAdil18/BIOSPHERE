...
export function ExperienceCard({ experience, index }: ExperienceCardProps) {
  return (
    ...
    >
      ...
      <div className="hidden md:block absolute left-1/2 top-0 w-4 h-4 rounded-full bg-gradient-to-br from-teal-500 to-cyan-500 -translate-x-1/2 z-10"></div>
      ...
      <div className={`md:flex items-start justify-between ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
        ...
        <div className={`md:w-[45%] ${index % 2 === 0 ? 'text-left md:text-left' : 'text-left md:text-right'}`}>
          <div className="bg-card p-6 rounded-xl border border-border hover:shadow-lg hover:border-teal-200 transition-all group">
            <h3 className="text-xl font-bold text-foreground group-hover:text-teal-600 transition-colors font-heading mb-1">
              {experience.role}
            </h3>
            <div className={`flex items-center gap-2 text-teal-600 text-sm font-medium mb-4 ${index % 2 === 0 ? 'md:justify-start' : 'md:justify-end'}`}>
