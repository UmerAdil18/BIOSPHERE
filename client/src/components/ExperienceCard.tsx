...
export function ExperienceCard({ experience, index }: ExperienceCardProps) {
  return (
    ...
    >
      ...
      <div className="hidden md:block absolute left-1/2 top-0 w-4 h-4 rounded-full bg-zinc-950 border-2 border-primary -translate-x-1/2 z-10"></div>
      ...
      <div className={`md:flex items-start justify-between ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
        ...
        <div className={`md:w-[45%] mt-2 md:mt-0 ${index % 2 === 0 ? 'text-left md:text-right' : 'text-left md:text-left'}`}>
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border border-zinc-800 bg-zinc-950/50 text-xs font-medium text-zinc-400 ${index % 2 === 0 ? 'md:mr-8' : 'md:ml-8'}`}>
